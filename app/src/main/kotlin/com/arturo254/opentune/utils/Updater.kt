...
object Updater {
    ...
    private suspend fun fetchReleasesNetwork(
        ...
        val response: HttpResponse =
            client.get("https://api.github.com/repos/Arturo254/Musiq/releases?per_page=$perPage") {
                headers {
                    append("Accept", "application/vnd.github+json")
                    append("User-Agent", "Musiq")
    ...
    private suspend fun resolveApkDownloadUrl(tagName: String): String {
        val fallback =
            "https://github.com/Arturo254/Musiq/releases/download/$tagName/$APK_ASSET_NAME"

        return runCatching {
            val response = client.get(
                "https://api.github.com/repos/Arturo254/Musiq/releases/tags/$tagName"
            ) {
                headers {
                    append("Accept", "application/vnd.github+json")
                    append("User-Agent", "Musiq")
    ...
    suspend fun getCommitHistory(count: Int = 20, branch: String = "master"): Result<List<GitCommit>> =
        runCatching {
            val response =
                client.get("https://api.github.com/repos/Arturo254/Musiq/commits?sha=$branch&per_page=$count") {
                    headers {
                        append("Accept", "application/vnd.github+json")
                        append("User-Agent", "Musiq")
                    }
    ...
    fun getLatestDownloadUrl(): String {
        ...
        return when (channel) {
            UpdateChannel.STABLE -> {
                "https://github.com/Arturo254/Musiq/releases/latest/download/$APK_ASSET_NAME"
            }
            UpdateChannel.NIGHTLY -> {
